            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div>
                    <div class="flex justify-between items-center mb-6">
                        ...
                        {{-- TOMBOL EDIT --}}
                        <button onclick="openEditModal()" class="text-sm font-bold text-primary hover:text-primary-dark bg-primary/10 px-3 py-1.5 rounded-lg transition">
                ...
                <div>
                    <h3 class="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                        <i class="fas fa-chart-pie text-primary"></i> Ringkasan Aktivitas
                    </h3>
                    ...
                    <div class="space-y-3">
                        <a href="{{ route('user.reservasi') }}" class="block w-full py-3 px-4 bg-primary text-white text-center rounded-xl font-bold hover:bg-primary-dark transition shadow-lg shadow-primary/20">
        ...
        <form action="{{ route('user.profile.update') }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="nama" value="{{ Auth::user()->nama }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary" required>
                </div>
                
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Email</label>
                    <input type="email" name="email" value="{{ Auth::user()->email }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary" required>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Nomor Telepon</label>
                    <input type="text" name="no_telepon" value="{{ Auth::user()->no_telepon }}" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary">
                </div>
            </div>
            ...
            <div class="mt-6 flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" class="px-6 py-2 bg-primary text-white font-bold rounded-lg hover:bg-primary-dark shadow-lg shadow-primary/20 transition">
                    Simpan Perubahan
                </button>
            </div>
        </form>
