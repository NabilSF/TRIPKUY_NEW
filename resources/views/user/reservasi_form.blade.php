    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
        ...
        <div class="bg-gray-50 p-4 rounded-xl mb-6 flex gap-4">
            <div class="w-24 h-24 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0">
                @php
                    $hotel = $kamar->hotel;
                    // Logika Gambar (Prioritas: Uploaded -> Bawaan -> Placeholder)
        ...
        <form action="{{ route('user.reservasi.store') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="check_in" class="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:border-[#2aa090] transition" required>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Check-out</label>
                    <input type="date" name="check_out" class="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:border-[#2aa090] transition" required>
                </div>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-bold text-gray-700 mb-2">Jumlah Kamar</label>
                <input type="number" name="jumlah_kamar" value="1" min="1" class="w-full border border-gray-300 rounded-lg p-3 focus:outline-none focus:border-[#2aa090] transition" required>
            </div>

            <button type="submit" class="w-full bg-[#2aa090] hover:bg-[#1f7a6e] text-white font-bold py-4 rounded-xl transition shadow-lg hover:shadow-[#2aa090]/30 transform hover:-translate-y-0.5">
                Bayar & Konfirmasi
            </button>
        </form>
    </div>
